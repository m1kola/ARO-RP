	...
	for _, tt := range []struct {
		...
		},
		{
			...
			mocks: func(kubernetescli *fake.Clientset) {
				kubernetescli.PrependReactor("create", "storageclasses", func(action ktesting.Action) (handled bool, ret runtime.Object, err error) {
					...
					return true, nil, errors.New("fake error from update of encrypted StorageClass")
				})
			},
			wantErr: "fake error from update of encrypted StorageClass",
		},
	} {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.wantNewSC {
				...
				// Old StorageClass is not longer default
